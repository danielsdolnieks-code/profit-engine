...
s=initial();s.common=32000;s.products[0]={...s.products[0],price:60,volume:1000,variable:30};s.products[1]={...s.products[1],price:40,volume:1000,variable:36,active:true};r=calculate(s);assert.equal(r.profit,2000);assert.equal(r.products[1].reported,-12000);s.allocation='revenue';let t=calculate(s);assert.equal(t.profit,r.profit);assert.notEqual(t.products[1].reported,r.products[1].reported);s.products[1].active=false;r=calculate(s);assert.equal(r.profit,-2000);assert.equal(r.fixed,32000);
